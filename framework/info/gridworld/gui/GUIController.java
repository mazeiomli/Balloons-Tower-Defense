...
//            	if(ifAdd)
     ...
     * Modified 2/17/2017 by Eric Li
     * Won't allow the yellow path to be clicked
     */
    private void locationClicked()
    {
        ...
        try{		
			Scanner inFile = new Scanner(new File("BalloonPath.txt"));
			while(inFile.hasNextInt()){
				int row = inFile.nextInt();
				int col = inFile.nextInt();
				if((new Location(row,col)).equals(loc)){
					return;
				}
			}
			inFile.close();
